...
async function connectionLogic() {
  ...
  Void.ev.on("messages.upsert", async (chatUpdate) => {
      ...
      if (citel.isGroup) {
        ...
        if (!group) {
          sck.insertOne({
            ...
            removeAdminSwitch: false,
            events: false,
            event: {
              goodBye: `Прощай @user из группы: @gname, осталось: @count человек`,
              welcome: `Привет @user! Добро пожаловать в @gname. Сейчас @count участников. Прочитай описание:\n\n@desc`
            },
          })
    ...
    if (citel.isGroup && group && group.banChat && !(citel.budy === '.разбанчат' || citel.budy === '.unbanchat' )) return
    await updateMessageCounter(citel, citel.sender, citel.chat, citel.pushName, sck1, sck)
    let text;
    try {
  ...
  Void.ev.on('group-participants.update', async (anu) => {
    try {
      groupMetadataCache.del(anu.id);
      const [group, metadata] = await Promise.all([
          sck.findOne({ id: anu.id }),
          Void.metadataGroup(anu.id)
      ]);
      if ( group && group.banChat ) return
      if (!group || !metadata) return;
      let botNumber;
      ...
      switch (anu.action) {
        ...
        case 'add':
          if (group.events) {
              const welcomePromises = anu.participants.map(num => {
                  const welcomeMessage = group.event.welcome ? group.event.welcome
                      .replace(/@user/gi, `@${num.split("@")[0]}`)
                      .replace(/@gname/gi, metadata.subject)
                      .replace(/@desc/gi, metadata.desc)
                      .replace(/@count/gi, totalmem)
                      : '';
                  return Void.sendMessage(anu.id, { text: welcomeMessage.trim(), mentions: [num] });
              });
        ...
        case 'remove':
          if (group.events) {
            const byePromises = anu.participants.map(num => {
              const byeMessage = group.event.goodBye ? group.event.goodBye
              .replace(/@user/gi, `@${num.split("@")[0]}`)
              .replace(/@gname/gi, metadata.subject)
              .replace(/@desc/gi, metadata.desc)
              .replace(/@count/gi, totalmem)
              : '';
          return Void.sendMessage(anu.id, { text: byeMessage.trim(), mentions: [num] });
      });
