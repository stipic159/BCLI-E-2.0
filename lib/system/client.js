...
const { smsg, loadCommands, findCommandByAlias, sck1, updateMessageCounter } = require('../index');
...
async function connectionLogic() {
  ...
  Void.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      // Обработка команды
      let cmdText = text;
      const [cmdName, ...args] = cmdText.slice(1).trim().split(/ +/);
      ...
      if (cmd) {
          ...
          if (cmd.isCreator && user.infoRoleUsers.status !== 'isCreator') {
              return bot.sendMessage(chatId, 'Команда доступна только создателю.');
          }
          if (cmd.isGroup && !citel.isGroup) {
              return bot.sendMessage(chatId, 'Эта команда доступна только в группах.');
          }
          if (cmd.isPrivate && citel.isGroup) {
              return bot.sendMessage(chatId, 'Эта команда доступна только в личных сообщениях.');
          }
          if (cmd.isAdmins) {
            const adminList = await getAdmin(Void, citel)
            if (!adminList.includes(citel.sender)) {
                return bot.sendMessage(citel.chat, 'Команда доступна только администраторам.');
            }
        }
        if (cmd.botAdmins) {
            const adminList = await getAdmin(Void, citel);
            ...
            if (!adminList.includes(botJid)) {
                return bot.sendMessage(citel.chat, 'У меня нет админских прав.');
            }
        }
          await cmd.execute(Void, citel);
