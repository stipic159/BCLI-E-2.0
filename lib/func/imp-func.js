...
async function updateMessageCounter(citel, userId, chatId, name, sck1, sck) {
    try {
        const currentDate = new Date();
        const user = await sck1.findOne({ id: userId })
        sck1.updateOne({ id: userId }, { $set: { name: name, lastSendMsg: currentDate }, $inc: { msg: 1, allMsg: 1 } })
        if (citel.isGroup) {
          const group = sck.findOne({ id: chatId })
          sck.updateOne({ id: chatId }, { $set: { lastSendMsg: currentDate }, $inc: { msg: 1, allMsg: 1 } })
          const groupLevelUpdatePromise = groupUpdateLevel(group, sck)
          await Promise.all([groupLevelUpdatePromise])
        }
        const levelUpdatePromise = updateLevel(user, sck1);
        await Promise.all([levelUpdatePromise]);
        console.log(`Данные для группы ${chatId} обновлены`)
...
const updateLevel = async (user, sck1) => {
  ...
  if (user.allMsg >= messagesPerLevel) {
...
const groupUpdateLevel = async (group, sck) => {
  const messagesPerLevel = group.level * 700;

  if (group.allMsg >= messagesPerLevel) {
    const nextLevel = group.level + 1

    await sck.updateOne({ id: group.id }, { $set: { level: nextLevel, allMsg: 0 } })
    console.log(`Уровень группы c id - ${group.id}, обновлен до ${nextLevel}`)
  }
}
